...
def _fully_connected_segment_text() -> dict[str, str]:
    return {
        ...
        "one_hot_label": "正解ラベルはone-hotで表します。正解クラスだけが一で、他のクラスはゼロです。",
...
def _backpropagation_segment_text() -> dict[str, str]:
    return {
        ...
        "hidden_weighted_return": "W2転置かけるデルタ2は、出力層の誤差を、接続重みに沿って隠れ層へ配り直す部分です。",
        "hidden_activation_gate": "シグマプライムは、隠れノードがその場所でどれだけ修正に反応できるかを表します。",
        "hidden_elementwise_product": "丸に点の掛け算は、同じ位置どうしを掛けるという意味です。ノードごとの調整になります。",
